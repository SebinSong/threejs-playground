...
const {
  WebGLRenderer, Scene, PerspectiveCamera, Vector3, Object3D, Color,
...
let axes, customPlane, plane, convex1, pointGroup
...
const cameraSettings = {
  ...
  // position: new Vector3(planeWidth * 1.1, Math.max(planeWidth, planeHeight) * 1.25, planeHeight * 1.1),
  // lookAt: [planeWidth/2, 0, planeHeight/2]
  position: new Vector3(planeWidth/2, 90, planeHeight*1.4),
  lookAt: [planeWidth/2, 30, planeHeight/2]
}
const colors = {
...
function initThree (canvasEl) {
  ...
  spotLight = new SpotLight(colors.light, 1.1)
  ...
  // objects
  const convexRefPoint = new Vector3(planeWidth/2, 30, planeHeight/2)
  const convexPoints = []
  const distanceLimit = 10
  const randomizePoint = () => randomSign() * Math.random() * distanceLimit

  pointGroup = new Group()
  const [pointGeo, pointMaterial] = [
    new SphereGeometry(0.1), new MeshLambertMaterial({ color: colors.objects[3] })
  ]

  for (let n=0; n<50; n++) {
    const p = new Vector3(
      randomizePoint(), randomizePoint(), randomizePoint()
    )

    const pointSphere = new Mesh(pointGeo, pointMaterial)
    pointSphere.position.copy(p)
    pointGroup.add(pointSphere)

    convexPoints.push(p)
  }

  convex1 = new CombineMaterial(
    new ConvexGeometry(convexPoints),
    [
      new MeshLambertMaterial({ color: colors.objects[2], transparent: true, opacity: 0.7,
        side: THREE.DoubleSide }),
      new MeshBasicMaterial({ color: '#9A9A9A', transparent: true, opacity: 0.35, wireframe: true })
    ], true
  )

  convex1.position.copy(convexRefPoint)
  pointGroup.position.copy(convexRefPoint)
...
function animate () {
  ...
  rotateObj(convex1, 0.5)
  rotateObj(pointGroup, 0.5)
